...
# =============================================================================
# E2E Test: excluded speaker is skipped at extraction (soft enforcement)
# =============================================================================

def skip_if_no_gemini():
    """This test only needs Gemini (claim extraction); skip if its key is missing."""
    if not (os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")):
        pytest.skip("Missing GEMINI_API_KEY / GOOGLE_API_KEY")
...
async def test_excluded_speaker_claim_is_dropped(cheap_models):
    ...
    Soft enforcement (LLM follows the prompt rule). Calls extraction directly on an
    already-resolved transcript (real names), so speaker-label resolution is out of scope.
    """
    skip_if_no_gemini()
    ...
    extractor = ClaimExtractor()
    # conftest sets ALLOW_MODEL_REQUESTS=False to block accidental API calls; this
    # integration test deliberately hits the real Gemini model, so allow it here.
