...
class TestFactCheckerCheckClaim:
    ...
    async def test_check_claim_with_context(self, mock_fact_checker, mock_create_agent):
        ...
        # Verify agent was called via stream (used by _invoke_with_trace)
    ...
    async def test_recursion_limit_retries_and_succeeds(self, mock_create_agent, mock_fact_check_response, mock_critique_async):
        """On GraphRecursionError, retries once with invoke and returns successful result."""
        from langgraph.errors import GraphRecursionError

        mock_agent = mock_create_agent.return_value

        # stream raises GraphRecursionError after yielding one intermediate state
        def mock_stream_raises(*args, **kwargs):
            yield {"messages": [MagicMock()]}
            raise GraphRecursionError("Recursion limit of 25 reached")
        mock_agent.stream = MagicMock(side_effect=mock_stream_raises)

        # invoke (retry) succeeds
        ...
        assert result["speaker"] == "Test Speaker"
        mock_agent.stream.assert_called_once()
        mock_agent.invoke.assert_called_once()  # retry was triggered

    async def test_recursion_limit_retry_also_fails(self, mock_create_agent):
        """On GraphRecursionError + retry failure, returns 'unklar' gracefully."""
        from langgraph.errors import GraphRecursionError
        ...
        def mock_stream_raises(*args, **kwargs):
            ...
            raise GraphRecursionError("Recursion limit of 25 reached")
        mock_agent.stream = MagicMock(side_effect=mock_stream_raises)
        mock_agent.invoke = MagicMock(side_effect=Exception("Retry also failed"))
...
class TestFactCheckerCheckClaims:
    ...
    async def test_check_claims_async_with_context(self, mock_fact_checker, mock_create_agent):
        ...
        # Agent should be called via stream for each claim
        mock_agent = mock_create_agent.return_value
